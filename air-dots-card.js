...
class AirDotsCard extends LitElement {
  ...
  _sensorCol(s, idx, theme) {
    ...
    // Awair Element "danger meter" semantic: more lit dots = worse.
    // Each dot has a fixed color by position (bottom → top:
    // green → yellow → orange → red → purple). Level N lights the
    // bottom N dots. The score column uses a different semantic
    // (more = better) since v0.9.1 because the score is inverted.
    return html`
